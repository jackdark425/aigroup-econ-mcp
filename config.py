...
class ConfigManager:
        ...
        try:
            if file_type == "auto":
                if filename.endswith(".json"):
                    ...
                    raise ConfigurationError(f"不支持的文件格式: {filename}")
            ...
            raise ConfigurationError(f"配置文件加载失败: {file_path}, 错误: {e}")
    ...
    def load_from_environment(self, prefix: str = "AIGROUP_ECON_") -> None:
        ...
        self.logger.info(f"环境变量配置加载成功: {prefix}")
    ...
    def get(self, key: str, default: Any = None) -> Any:
        ...
        if key in self._defaults and self._defaults[key]["required"]:
            raise ConfigurationError(f"必需配置缺失: {key}")
    ...
    def set(self, key: str, value: Any, description: str = "") -> None:
        ...
        self._config[key] = ConfigValue(
            value=value,
            source=ConfigSource.FILE,  # 手动设置视为文件来源
            key=key,
            description=description,
            required=required,
            validated=validated
    ...
    def save_to_file(self, filename: str, file_type: str = "json") -> None:
        ...
        try:
            ...
            for key, config_value in self._config.items():
                if config_value.source != ConfigSource.ENV:  # 不保存环境变量
            ...
            raise ConfigurationError(f"配置文件保存失败: {file_path}, 错误: {e}")
    ...
    def validate_boolean(value: Any) -> bool:
        ...
        if isinstance(value, str):
            if value.lower() in ("true", "1", "yes", "on"):
                ...
                return False
        raise ValueError("必须是布尔值 (true/false, 1/0, yes/no)")
...
class EconometricConfig:
    ...
    def _set_defaults(self) -> None:
        ...
        defaults = {
            ...
            "cache_max_size": (1000, "缓存最大大小", False),
            
            # 监控配置
            "monitoring_enabled": (True, "是否启用监控", False),
            ...
            "default_alpha": (1.0, "默认正则化参数", False),
            
            # 数据验证
            "min_sample_size": (10, "最小样本量", False),
            "max_feature_count": (100, "最大特征数", False),
