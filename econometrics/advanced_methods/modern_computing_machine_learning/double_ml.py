...
class DoubleML:
    ...
    def fit(
        self, X: np.ndarray | pd.DataFrame, y: np.ndarray | pd.Series, d: np.ndarray | pd.Series
    ) -> "DoubleML":
        """
        ...
        crit_val = 1.96  # 95% CI
        self.ci = (self.effect - crit_val * self.se, self.effect + crit_val * self.se)
    ...
    def predict(self, X: np.ndarray | pd.DataFrame) -> np.ndarray:
        ...
        """
        return (
            np.full(X.shape[0], self.effect)
            if hasattr(X, "shape")
            else np.full(len(X), self.effect)
        )
...
def double_ml_analysis(
    ...
    dml_model = DoubleML(treatment_type=treatment_type, n_folds=n_folds, random_state=random_state)
    dml_model.fit(X, y, d)
