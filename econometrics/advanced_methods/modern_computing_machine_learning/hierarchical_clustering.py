...
class EconHierarchicalClustering:
    ...
    def fit(self, X: np.ndarray | pd.DataFrame) -> "EconHierarchicalClustering":
        """
    ...
    def predict(self, X: np.ndarray | pd.DataFrame = None) -> np.ndarray:
        """
        Get cluster labels
    ...
    def evaluate(self, X: np.ndarray | pd.DataFrame) -> dict[str, float]:
        """
    ...
    def plot_dendrogram(
        ...
        if self.linkage_matrix is None:
            raise ValueError(
                "No linkage matrix available. Please fit the model first or provide data."
            )

        # Create plot
        fig, ax = plt.subplots(figsize=figsize)
        dendrogram(self.linkage_matrix, truncate_mode=truncate_mode, p=p, ax=ax)
...
def hierarchical_clustering_analysis(
    ...
    hc_model = EconHierarchicalClustering(n_clusters=n_clusters, linkage=linkage, metric=metric)
    labels = hc_model.fit_predict(X)

    # Evaluate clustering
    metrics = hc_model.evaluate(X)

    return {"model": hc_model, "labels": labels, "metrics": metrics, "X": X}
