...
class EconKMeans:
    ...
    def fit(self, X: np.ndarray | pd.DataFrame) -> "EconKMeans":
        """
    ...
    def visualize_clusters(
        self, X: np.ndarray | pd.DataFrame, max_features: int = 10, figsize: tuple = (12, 8)
    ) -> Optional:
        """
        ...
        if hasattr(self.model, "cluster_centers_"):
            centers_pca = (
                pca.transform(self.model.cluster_centers_)
                if X_scaled.shape[1] > 2
                else self.model.cluster_centers_
            )
            ax.scatter(
                centers_pca[:, 0], centers_pca[:, 1], c="red", marker="x", s=200, linewidths=3
            )
