...
class EconSVM:
    ...
    def __init__(
        ...
        if problem_type == "regression":
            ...
            self.model = SVC(
                kernel=kernel, C=C, gamma=gamma, random_state=random_state, probability=True
            )
        else:
            raise ValueError("problem_type must be either 'regression' or 'classification'")

    def fit(self, X: np.ndarray | pd.DataFrame, y: np.ndarray | pd.Series) -> "EconSVM":
        """
        Fit the SVM model
...
def svm_analysis(
    ...
    svm_model = EconSVM(
        problem_type=problem_type, kernel=kernel, C=C, gamma=gamma, random_state=random_state
    )
