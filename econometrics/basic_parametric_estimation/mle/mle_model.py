...
def _normal_mle(
    data: np.ndarray, initial_params: list[float] | None, confidence_level: float
) -> MLEResult:
    """正态分布最大似然估计"""
    # 使用样本均值和标准差作为初始估计
    mu_hat = np.mean(data)
    ...
    try:
        ...
        raise ValueError(f"正态分布MLE估计失败: {str(e)}") from e
...
def _poisson_mle(
    ...
    try:
        ...
        raise ValueError(f"泊松分布MLE估计失败: {str(e)}") from e
...
def _exponential_mle(
    ...
    try:
        ...
        raise ValueError(f"指数分布MLE估计失败: {str(e)}") from e
