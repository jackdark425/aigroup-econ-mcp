...
class IVResult(BaseModel):
    ...
    first_stage_f_stat: float | None = Field(None, description="第一阶段F统计量")
...
def instrumental_variables_2sls(
    ...
    for i in range(k_z):
        var_name = (
            instrument_names[i] if instrument_names and i < len(instrument_names) else f"z{i + 1}"
        )
    ...
    y_var = "y"
    x_vars = [
        feature_names[i] if feature_names and i < len(feature_names) else f"x{i + 1}"
        for i in range(k_x)
    ]
    z_vars = [
        instrument_names[i] if instrument_names and i < len(instrument_names) else f"z{i + 1}"
