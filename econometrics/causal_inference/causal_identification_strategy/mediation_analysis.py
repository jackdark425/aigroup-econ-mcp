...
class MediationResult(BaseModel):
    ...
    sobel_test_statistic: float | None = Field(None, description="Sobel检验统计量")
...
def mediation_analysis(
    ...
    """
    # 构建数据
    df = pd.DataFrame({"outcome": outcome, "treatment": treatment, "mediator": mediator})
    ...
    # Sobel检验标准误
    indirect_effect_se = np.sqrt((alpha**2) * (beta1_se**2) + (beta1**2) * (alpha_se**2))
