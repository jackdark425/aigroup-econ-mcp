...
def variance_decomposition(
    values: list[float], groups: list[str], group_names: list[str] | None = None
    ...
    # SS_between = Σnᵢ(ȳᵢ - ȳ)²
    ss_between = sum(
        group_sizes[str(gid)] * (group_means[str(gid)] - grand_mean) ** 2 for gid in unique_groups
    )

    # SS_within = Σ(nᵢ - 1)sᵢ²
    ss_within = sum(
        (group_sizes[str(gid)] - 1) * group_variances[str(gid)] for gid in unique_groups
    )
