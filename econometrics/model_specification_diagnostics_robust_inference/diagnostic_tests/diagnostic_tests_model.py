...
class DiagnosticTestsResult(BaseModel):
    ...
    het_breuschpagan_stat: float | None = Field(None, description="Breusch-Pagan异方差检验统计量")
    het_breuschpagan_pvalue: float | None = Field(None, description="Breusch-Pagan异方差检验p值")
    het_white_stat: float | None = Field(None, description="White异方差检验统计量")
    het_white_pvalue: float | None = Field(None, description="White异方差检验p值")
    dw_statistic: float | None = Field(None, description="Durbin-Watson自相关检验统计量")
    jb_statistic: float | None = Field(None, description="Jarque-Bera正态性检验统计量")
    jb_pvalue: float | None = Field(None, description="Jarque-Bera正态性检验p值")
    vif_values: list[float] | None = Field(None, description="方差膨胀因子(VIF)")
    feature_names: list[str] | None = Field(None, description="特征名称")
...
def diagnostic_tests(
    ...
    return DiagnosticTestsResult(
        ...
        if constant and feature_names and len(feature_names) > 1
        else feature_names,
    )
