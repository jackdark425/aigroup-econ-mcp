...
def two_stage_least_squares(
    ...
    equation_names: list[str] | None = None,  # 方程名称列表
    instrument_names: list[str] | None = None,  # 工具变量名称列表
    constant: bool = True,
    ...
    for i in range(n_equations):
        if len(y_data[i]) != n_obs:
            raise ValueError(
                f"第{i + 1}个方程的因变量观测数量({len(y_data[i])})必须与其他方程相同({n_obs})"
            )
    ...
    for i in range(n_equations):
        ...
        # 方程名称
        eq_name = (
            equation_names[i] if equation_names and i < len(equation_names) else f"equation_{i + 1}"
        )
