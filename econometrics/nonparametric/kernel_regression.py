...
class KernelRegressionResult(BaseModel):
    ...
    aic: float | None = Field(None, description="AIC信息准则")
...
def kernel_regression(
    ...
    bandwidth: list[float] | None = None,
    ...
    try:
        if bandwidth is None:
            ...
            kr = KernelReg(
                endog=y, exog=X, var_type=var_type, reg_type="ll", bw=np.array(bandwidth)
            )
    except Exception as e:
        raise ValueError(f"核回归模型构建失败: {str(e)}") from e
