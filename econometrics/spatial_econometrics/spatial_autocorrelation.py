...
def morans_i_test(
    ...
    if not ESDA_AVAILABLE:
        raise ImportError("esda库未安装。请运行: pip install esda\n或: pip install pysal")
...
"""
        ...
        raise ValueError(f"Moran's I检验失败: {str(e)}") from e


def gearys_c_test(
    values: list[float], neighbors: dict, weights: dict | None = None, permutations: int = 999
) -> GearysCResult:
    """
    Geary's C 空间自相关检验
...
"""
        ...
        raise ValueError(f"Geary's C检验失败: {str(e)}") from e
...
def local_morans_i(
    ...
    # 识别显著位置
    significant_locations = [i for i, p in enumerate(p_values) if p < significance_level]
