...
try:
    from libpysal.weights import KNN, DistanceBand, Kernel, W
...
def create_spatial_weights(
    ...
    if not LIBPYSAL_AVAILABLE:
        raise ImportError("libpysal库未安装。请运行: pip install libpysal\n或: pip install pysal")
    ...
    if weight_type == "queen":
        ...
        raise ValueError(
            f"不支持的权重类型: {weight_type}。支持的类型: queen, rook, knn, distance, kernel"
        )
