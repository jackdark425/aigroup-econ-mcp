...
class TobitModel:
    ...
    def fit(self, X, y):
        ...
        class TobitLikelihoodModel(GenericLikelihoodModel):
            ...
            def loglikeobs(self, params):
                ...
                if self.upper_bound is None:
                    ...
                    ll = np.zeros_like(self.endog)
                    # 左截断观测的对数似然
                    ll[left_censored] = stats.norm.logcdf(
                        (self.lower_bound - xb[left_censored]) / sigma
                    )
                    # 右截断观测的对数似然
                    ll[right_censored] = stats.norm.logsf(
                        (self.upper_bound - xb[right_censored]) / sigma
                    )
                    # 未截断观测的对数似然
                    ll[uncensored] = -0.5 * np.log(2 * np.pi * sigma**2) - 0.5 * z[uncensored] ** 2
        ...
        self.model_ = TobitLikelihoodModel(
            endog=y, exog=X_with_const, lower_bound=self.lower_bound, upper_bound=self.upper_bound
        )
        ...
        initial_params = np.concatenate(
            [
                ...
                [
                    np.std(y[y > self.lower_bound]) if self.upper_bound is None else np.std(y)
                ],  # sigma
            ]
        )
    ...
    def predict(self, X):
        ...
        if self.upper_bound is None:
            ...
            lambda_lower = stats.norm.pdf(z_lower) / np.clip(
                stats.norm.cdf(z_upper) - stats.norm.cdf(z_lower), 1e-10, 1
            )
            lambda_upper = stats.norm.pdf(z_upper) / np.clip(
                stats.norm.cdf(z_upper) - stats.norm.cdf(z_lower), 1e-10, 1
            )
...
class HeckmanModel:
    ...
    def fit(self, X_select, Z, y, s):
        ...
        mills_ratio = stats.norm.pdf(X_select_linpred) / np.clip(
            stats.norm.cdf(X_select_linpred), 1e-10, 1 - 1e-10
        )
        # 对于未被选择的样本，米尔斯比率为0
