...
class ARIMAResult(BaseModel):
    ...
    r_squared: float | None = Field(None, description="R方")
    adj_r_squared: float | None = Field(None, description="调整R方")
    n_obs: int = Field(..., description="观测数量")
    forecast: list[float] | None = Field(None, description="预测值")


def arima_model(
    data: list[float], order: tuple = (1, 1, 1), forecast_steps: int = 1
) -> ARIMAResult:
    """
    ARIMA模型实现
    ...
    try:
        ...
        r_squared = (
            float(getattr(fitted_model, "rsquared", None))
            if hasattr(fitted_model, "rsquared")
            else None
        )
        adj_r_squared = (
            float(getattr(fitted_model, "rsquared_adj", None))
        ...
        # 出现错误时抛出异常
        raise ValueError(f"ARIMA模型拟合失败: {str(e)}") from e
