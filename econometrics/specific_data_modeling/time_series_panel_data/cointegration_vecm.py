...
class CointegrationResult(BaseModel):
    ...
    critical_values: dict | None = Field(None, description="临界值")
    cointegrating_vectors: list[list[float]] | None = Field(None, description="协整向量")
    rank: int | None = Field(None, description="协整秩")
...
class VECMResult(BaseModel):
    ...
    std_errors: list[list[float]] | None = Field(None, description="系数标准误矩阵")
    t_values: list[list[float]] | None = Field(None, description="t统计量矩阵")
    p_values: list[list[float]] | None = Field(None, description="p值矩阵")
    alpha: list[list[float]] | None = Field(None, description="调整系数矩阵")
    beta: list[list[float]] | None = Field(None, description="协整向量矩阵")
    gamma: list[list[float]] | None = Field(None, description="短期系数矩阵")
    log_likelihood: float | None = Field(None, description="对数似然值")
    aic: float | None = Field(None, description="赤池信息准则")
    bic: float | None = Field(None, description="贝叶斯信息准则")
    n_obs: int = Field(..., description="观测数量")


def engle_granger_cointegration_test(
    data: list[list[float]], variables: list[str] | None = None
) -> CointegrationResult:
    """
    Engle-Granger协整检验实现
    ...
    try:
        ...
        if data_array.shape[0] >= 2:
            ...
            if x_variables.shape[0] == 1:
                ...
                try:
                    ...
                    raise ValueError(f"多变量协整检验计算失败: {str(e)}") from e
        ...
        # 出现错误时抛出异常
        raise ValueError(f"Engle-Granger协整检验失败: {str(e)}") from e
    ...
    try:
        ...
        if johansen_result.cvt is not None and johansen_result.lr1 is not None:
            critical_values = (
                ...
                if johansen_result.cvt.shape[1] > 1
                else johansen_result.cvt[:, 0]
            )  # 5%显著性水平
            rank = (
                int(sum(johansen_result.lr1 > critical_values))
                if len(johansen_result.lr1) > 0 and len(critical_values) > 0
                else 0
            )
        ...
        # 出现错误时抛出异常
        raise ValueError(f"Johansen协整检验失败: {str(e)}") from e


def vecm_model(
    data: list[list[float]], coint_rank: int = 1, variables: list[str] | None = None
) -> VECMResult:
    """
    向量误差修正模型(VECM)实现
    ...
    try:
        ...
        # 出现错误时抛出异常
        raise ValueError(f"VECM模型拟合失败: {str(e)}") from e
