    ...
    try:
        ...
        if use_linearmodels:
            ...
            try:
                ...
                p_values = [
                    2 * (1 - t.cdf(np.abs(t_val), len(dy) - n_params)) for t_val in t_values
                ]

                # 置信区间
                t_critical = t.ppf(0.975, len(dy) - n_params)
                conf_int_lower = [
                    p - t_critical * se for p, se in zip(params, std_errors, strict=False)
                ]
                conf_int_upper = [
                    p + t_critical * se for p, se in zip(params, std_errors, strict=False)
                ]
    ...
    try:
        ...
        if use_linearmodels:
            ...
            try:
                ...
                p_values = [
                    2 * (1 - t.cdf(np.abs(t_val), len(y_sys) - n_params)) for t_val in t_values
                ]
