...
class ExponentialSmoothingResult(BaseModel):
    ...
    smoothing_level: float | None = Field(None, description="水平平滑参数")
    smoothing_trend: float | None = Field(None, description="趋势平滑参数")
    smoothing_seasonal: float | None = Field(None, description="季节平滑参数")
    ...
    sse: float | None = Field(None, description="误差平方和")
    mse: float | None = Field(None, description="均方误差")
    rmse: float | None = Field(None, description="均方根误差")
    mae: float | None = Field(None, description="平均绝对误差")
    n_obs: int = Field(..., description="观测数量")
    forecast: list[float] | None = Field(None, description="预测值")
...
def exponential_smoothing_model(
    ...
    try:
        ...
        # 获取参数
        smoothing_level = (
            float(fitted_model.params["smoothing_level"])
            if "smoothing_level" in fitted_model.params
            else None
        )
        smoothing_trend = (
            float(fitted_model.params["smoothing_trend"])
            if "smoothing_trend" in fitted_model.params
            else None
        )
        smoothing_seasonal = (
            float(fitted_model.params["smoothing_seasonal"])
            if "smoothing_seasonal" in fitted_model.params
            else None
        )

        # 提取所有参数作为系数
        coefficients = []
        for _param_name, param_value in fitted_model.params.items():
            # 确保参数值有效
        ...
        # 获取信息准则
        aic = (
            float(fitted_model.aic)
            if hasattr(fitted_model, "aic") and np.isfinite(fitted_model.aic)
            else None
        )
        bic = (
            float(fitted_model.bic)
            if hasattr(fitted_model, "bic") and np.isfinite(fitted_model.bic)
            else None
        )
        ...
        # 出现错误时抛出异常
        raise ValueError(f"指数平滑模型拟合失败: {str(e)}") from e
