...
class GARCHResult(BaseModel):
    ...
    volatility: list[float] | None = Field(None, description="波动率序列")
    persistence: float | None = Field(None, description="持续性参数")
    n_obs: int = Field(..., description="观测数量")


def garch_model(data: list[float], order: tuple[int, int] = (1, 1)) -> GARCHResult:
    """
    GARCH模型实现 - 使用自定义实现
    ...
    try:
        ...
        # 出现错误时抛出异常
        raise ValueError(f"GARCH模型拟合失败: {str(e)}") from e
        ...
        try:
            ...
            if result.success:
                ...
                for i in range(1, len(returns)):
                    h_opt[i] = (
                        omega_opt + alpha_opt * returns_squared[i - 1] + beta_opt * h_opt[i - 1]
                    )
