...
class PanelDiagnosticResult(BaseModel):
    ...
    significant: bool | None = Field(None, description="是否显著")
    recommendation: str | None = Field(None, description="建议")
...
def pooling_f_test(
    pooled_ssrs: float, fixed_ssrs: float, n_individuals: int, n_params: int, n_obs: int
) -> PanelDiagnosticResult:
    """
...
def lm_test(
    pooled_ssrs: float, random_ssrs: float, n_individuals: int, n_periods: int
) -> PanelDiagnosticResult:
    """
    LM检验实现（Pooled vs RE）
...
def within_correlation_test(residuals: list[list[float]]) -> PanelDiagnosticResult:
    """
    组内相关性检验实现
