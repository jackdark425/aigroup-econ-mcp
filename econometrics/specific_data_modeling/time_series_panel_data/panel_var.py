...
class PanelVARResult(BaseModel):
    ...
    individual_effects: list[float] | None = Field(None, description="个体效应")
    time_effects: list[float] | None = Field(None, description="时间效应")
    r_squared: float | None = Field(None, description="R方")
    adj_r_squared: float | None = Field(None, description="调整R方")
    f_statistic: float | None = Field(None, description="F统计量")
    f_p_value: float | None = Field(None, description="F统计量p值")
