...
class TimeVaryingParameterResult(BaseModel):
    ...
    transition_function: str | None = Field(None, description="转换函数")
    threshold_values: list[float] | None = Field(None, description="门限值")
    transition_coefficients: list[float] | None = Field(None, description="转换系数")
...
def markov_switching_model(
    y_data: list[float], x_data: list[list[float]], n_regimes: int = 2
) -> TimeVaryingParameterResult:
    """
    马尔科夫转换模型实现
