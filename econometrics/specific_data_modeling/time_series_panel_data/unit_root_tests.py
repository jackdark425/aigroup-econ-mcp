...
class UnitRootTestResult(BaseModel):
    ...
    lags: int | None = Field(None, description="滞后阶数")
    stationary: bool | None = Field(
        None,
        description="是否平稳 (ADF/PP: p<0.05为平稳; KPSS: p<0.05为非平稳，但接口统一返回p<0.05为平稳)",
    )
    n_obs: int = Field(..., description="观测数量")


def adf_test(
    data: list[float], max_lags: int | None = None, regression_type: str = "c"
) -> UnitRootTestResult:
    """
    ...
    try:
        ...
        # 出现错误时抛出异常
        raise ValueError(f"ADF检验失败: {str(e)}") from e


def pp_test(data: list[float], regression_type: str = "c") -> UnitRootTestResult:
    """
    ...
    try:
        ...
        # 出现错误时抛出异常
        raise ValueError(f"PP检验失败: {str(e)}") from e


def kpss_test(data: list[float], regression_type: str = "c") -> UnitRootTestResult:
    """
    KPSS 检验实现
    ...
    try:
        ...
        # 出现错误时抛出异常
        raise ValueError(f"KPSS检验失败: {str(e)}") from e
