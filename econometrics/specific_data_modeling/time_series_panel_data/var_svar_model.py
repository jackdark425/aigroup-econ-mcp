...
class VARResult(BaseModel):
    ...
    fpe: float | None = Field(None, description="最终预测误差")
    hqic: float | None = Field(None, description="汉南-奎因信息准则")
...
def var_model(
    data: list[list[float]], lags: int = 1, variables: list[str] | None = None
) -> VARResult:
    """
    向量自回归(VAR)模型实现
    ...
    try:
        ...
        # 出现错误时抛出异常
        raise ValueError(f"VAR模型拟合失败: {str(e)}") from e
    ...
    try:
        ...
        if a_matrix is not None:
            try:
                ...
                raise ValueError(f"A矩阵处理失败: {str(e)}") from e
        ...
        if b_matrix is not None:
            try:
                ...
                raise ValueError(f"B矩阵处理失败: {str(e)}") from e
        ...
        irf_result = fitted_model.irf(10)
        irf = (
            irf_result.irfs.flatten().tolist()
            if hasattr(irf_result, "irfs") and irf_result.irfs is not None
            else None
        )
        ...
        fevd_result = fitted_model.fevd(10)
        fevd = (
            fevd_result.decomp.flatten().tolist()
            if hasattr(fevd_result, "decomp") and fevd_result.decomp is not None
            else None
        )
        ...
        # 出现错误时抛出异常
        raise ValueError(f"SVAR模型拟合失败: {str(e)}") from e
