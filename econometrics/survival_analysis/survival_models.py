...
class KaplanMeierResult(BaseModel):
    ...
    median_survival_time: float | None = Field(None, description="中位生存时间")
...
def kaplan_meier_estimation_simple(
    durations: list[float], event_observed: list[int], confidence_level: float = 0.95
) -> KaplanMeierResult:
    """
...
def cox_regression_simple(
    ...
    ci_lower = [
        np.exp(coef - z_critical * se) for coef, se in zip(coefficients, std_errors, strict=False)
    ]
    ci_upper = [
        np.exp(coef + z_critical * se) for coef, se in zip(coefficients, std_errors, strict=False)
    ]
...
风险比估计:
"""
    for _i, (name, hr, coef, se, z, p, lower, upper) in enumerate(
        zip(
            feature_names,
            hazard_ratios,
            coefficients,
