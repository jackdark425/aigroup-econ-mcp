...
def test_instrumental_variables():
    ...
    try:
        result = instrumental_variables_2sls(
            y=y.tolist(), x=x.reshape(-1, 1).tolist(), instruments=z.reshape(-1, 1).tolist()
        )
...
def test_control_function():
    ...
    try:
        result = control_function_approach(
            y=y.tolist(), x=x.tolist(), z=np.column_stack([z1, z2]).tolist()
        )
        print(f"  系数: {result.estimate:.4f}")
        print(f"  标准误: {result.std_error:.4f}")
        print(f"  p值: {result.p_value:.4f}")
...
def test_fixed_effects():
    ...
    for i in range(n):
        ...
        y_value = (
            1 + 2 * x[i][0] + 1.5 * x[i][1] + entity_effects[entity_idx] + np.random.normal(0, 0.5)
        )
        y.append(y_value)

    try:
        result = fixed_effects_model(y=y, x=x, entity_ids=entity_ids, time_periods=time_periods)
        print(f"  系数: {result.estimate:.4f}")
        print(f"  标准误: {result.std_error:.4f}")
        print(f"  p值: {result.p_value:.4f}")
        print("  ✓ 固定效应模型测试通过\n")
...
def test_random_effects():
    ...
    try:
        result = random_effects_model(y=y, x=x, entity_ids=entity_ids, time_periods=time_periods)
        print(f"  系数: {result.estimate:.4f}")
        print(f"  标准误: {result.std_error:.4f}")
        print(f"  p值: {result.p_value:.4f}")
        print("  ✓ 随机效应模型测试通过\n")
...
def test_first_difference():
    ...
    try:
        result = first_difference_model(y=y.tolist(), x=x.tolist(), entity_ids=entity_ids)
        print(f"  系数: {result.estimate:.4f}")
        print(f"  标准误: {result.std_error:.4f}")
        print(f"  p值: {result.p_value:.4f}")
        print("  ✓ 一阶差分模型测试通过\n")
...
def test_hausman_test():
    ...
    try:
        result = hausman_test(y=y, x=x.tolist(), entity_ids=entity_ids, time_periods=time_periods)
        if hasattr(result, "hausman_statistic") and result.hausman_statistic >= 0:
            ...
            print(
                f"  ✗ Hausman检验返回无效统计量: {result.hausman_statistic if hasattr(result, 'hausman_statistic') else 'None'}\n"
            )
    except Exception as e:
...
def test_difference_in_differences():
    ...
    try:
        result = difference_in_differences(
            treatment=treatment, time_period=time_period, outcome=outcome
        )
...
def test_regression_discontinuity():
    ...
    try:
        result = regression_discontinuity(
            running_variable=running_variable, outcome=outcome, cutoff=cutoff, bandwidth=0.5
        )
...
def test_propensity_score_matching():
    ...
    # 结果变量
    outcome = (
        2 + 1.5 * np.array(treatment) + 0.8 * x1 + 0.5 * x2 + np.random.normal(0, 1, n)
    ).tolist()
...
def test_mediation_analysis():
    ...
    # 中介变量
    mediator = (
        1 + 0.8 * np.array(treatment) + 0.3 * x1 + 0.2 * x2 + np.random.normal(0, 1, n)
    ).tolist()

    # 结果变量
    outcome = (
        2
        + 1.2 * np.array(treatment)
        + 0.7 * np.array(mediator)
        + 0.4 * x1
        + 0.3 * x2
...
def test_moderation_analysis():
    ...
    outcome = (
        2
        + 1.2 * np.array(predictor)
        + 0.8 * np.array(moderator)
