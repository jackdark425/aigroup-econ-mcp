...
class TestAllCausalMethods(unittest.TestCase):
    ...
    def test_first_difference_model(self):
        ...
        # 时间标识
        [f"period_{t}" for _ in range(n_entities) for t in range(n_periods)]
    ...
    def test_hausman_test(self):
        ...
        # 执行Hausman检验
        try:
            result = hausman_test(y=y, x=x, entity_ids=entity_ids, time_periods=time_periods)
    ...
    def test_difference_in_differences(self):
        ...
        time_period = np.concatenate(
            [np.zeros(50), np.ones(50), np.zeros(50), np.ones(50)]
        ).tolist()
