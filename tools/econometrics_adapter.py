...
# 导入核心算法实现
from econometrics.basic_parametric_estimation.gmm.gmm_model import GMMResult as CoreGMMResult
from econometrics.basic_parametric_estimation.gmm.gmm_model import (
    gmm_estimation as core_gmm_estimation,
)
from econometrics.basic_parametric_estimation.mle.mle_model import MLEResult as CoreMLEResult
from econometrics.basic_parametric_estimation.mle.mle_model import (
    mle_estimation as core_mle_estimation,
)
from econometrics.basic_parametric_estimation.ols.ols_model import OLSResult as CoreOLSResult
...
class DataValidator:
    ...
    @staticmethod
    def validate_ols_gmm_data(
        y_data: list[float], x_data: list[list[float]], feature_names: list[str] | None = None
    ):
        """验证OLS和GMM数据格式"""
        ...
        if x_data:
            ...
            for i, row in enumerate(x_data):
                if len(row) != first_row_len:
                    raise ValueError(
                        f"自变量第{i}行长度({len(row)})与第一行长度({first_row_len})不一致"
                    )

        # 验证feature_names
        if feature_names and len(feature_names) != len(x_data[0]) if x_data else 0:
            raise ValueError(
                f"特征名称数量({len(feature_names)})与自变量列数({len(x_data[0]) if x_data else 0})不一致"
            )

    @staticmethod
    def convert_to_2d_list(data: list[float] | list[list[float]]) -> list[list[float]]:
        """将数据转换为二维列表格式"""
        if not data:
            return []
