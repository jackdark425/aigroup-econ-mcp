...
def spline_regression_adapter(
    ...
    result: SplineRegressionResult = spline_regression(
        y_data=y_data, x_data=x_data, n_knots=n_knots, degree=degree, knots=knots
    )
...
def gam_adapter(
    ...
    result: GAMResult = gam_model(
        y_data=y_data, x_data=x_data, problem_type=problem_type, n_splines=n_splines, lam=lam
    )
