...
class TimeSeriesPanelDataAdapter:
    ...
    def exponential_smoothing_model(
        ...
        if output_format == "json":
            ...
            try:
                formatted = OutputFormatter.format_exponential_smoothing_result(
    ...
    def panel_diagnostics(
        ...
        if test_type == "hausman":
            ...
            result = core_hausman_test(
                fe_coefficients, re_coefficients, fe_covariance, re_covariance
            )
            ...
            if not all(
                [pooled_ssrs is not None, random_ssrs is not None, n_individuals, n_periods]
            ):
                raise ValueError(
                    "LM test requires pooled_ssrs, random_ssrs, n_individuals, n_periods"
                )
    ...
    def time_varying_parameter_models(
        ...
        if output_format == "json":
            ...
            try:
                formatted = OutputFormatter.format_time_varying_parameter_result(
                    result, output_format
                )
            except Exception as e:
                formatted = json.dumps(result.dict(), ensure_ascii=False, indent=2)
